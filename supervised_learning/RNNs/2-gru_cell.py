...
class GRUCell():
    ...
    def forward(self, h_prev, x_t):
        """
        toute la diffuculté est d'ecrire efficacement les
        equations de la cellule GRU
        """
        ...
        out_h = np.tanh(np.dot(np.concatenate((out_Reset, x_t), axis=1),
                               self.Wh) + self.bh)

        # print("out_Reset", out_Reset.shape)
        # print("out_Update", out_Update.shape)
        # print("out_h", out_h.shape)
        # h_i0 est entre h-prev et h_next
