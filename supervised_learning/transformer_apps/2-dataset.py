...
class Dataset():
    ...
    def encode(self, pt, en):
        ...
        pt_tokens = [nouveau_cls_id] + self.tokenizer_pt.encode(pt, add_special_tokens=False) + [nouveau_sep_id]
        en_tokens = [nouveau_cls_id] + self.tokenizer_en.encode(en, add_special_tokens=False) + [nouveau_sep_id]
        #print("encode en", self.tokenizer_en.encode(en))
        return pt_tokens, en_tokens

    def tf_encode(self, pt, en):
        encoder = tf.py_function(func=self.encode, inp=[pt, en], Tout=[tf.int64, tf.int64])
        ...
        #print ("encoder TF",encoder.tokeniz)