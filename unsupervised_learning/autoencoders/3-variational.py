...
def autoencoder(input_dims, hidden_layers, latent_dims):
    ...
    # premier modele
    encoder = keras.models.Model(inputs=encoder_input,
                                 outputs=[z_mu, z_sigma, z],
                                 name='encoder')
...
def sample_z(args):
    ...
    """
    z_mu, z_sigma = args
    eps = keras.backend.random_normal(shape=(keras.backend.shape(z_mu)[0],
                                             keras.backend.int_shape(z_mu)[1]))
