...
class Simple_GAN(keras.Model):
    """
    This is the fisrt Class for task0
    """
    def __init__(self, generator, discriminator,
        ...
        self.generator.loss = lambda x: tf.keras.losses.\
            MeanSquaredError()(x, tf.ones(x.shape))
        ...
        self.discriminator.loss = lambda x, y: tf.keras.losses.\
            MeanSquaredError()(x, tf.ones(x.shape))\
            + tf.keras.losses.MeanSquaredError()(y, -1*tf.ones(y.shape))

        self.discriminator.optimizer = keras.optimizers.\
            Adam(learning_rate=self.learning_rate,
    ...
    def train_step(self, useless_argument):
        """
        Effectue une itération de l'entraînement du GAN :
        plusieurs mises à jour du discriminateur
